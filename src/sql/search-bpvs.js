/**
 * SQL builders for the batched PVS (Principal Variation Search) engine.
 * This is the primary high-performance search strategy used by the
 * DuckDB engine.  Implements the iterative, table-based search loop
 * using temporary tables for the frontier, minimax back-propagation,
 * transposition-table merging, quiescence search (QS), and pruning
 * heuristics (RFP, FFP, LMR, LMP, null-move pruning).
 *
 * Session lifecycle (table creation, root insertion, cleanup) is
 * managed by sessions.js; all search-algorithm SQL builders live here.
 */
...
import { getIsBitSetSQL, getOrSQL, getXorSQL, getBitIndexSQL, getNextValSQL, getZobristHashSQL } from './utils.js';
...
export function getLMPFilterSQL(options, maxDepth) {
    if (options.useLMP === false) {
        return '';
    }
    // Limit search to the first N ordered quiet moves at shallow plies (depth >= maxDepth - 2).
    // Captures, promotions, checks, and gives-checks bypass this pruning.
    return `WHERE (depth_val = 0 OR rnk <= (12 + (CAST(${maxDepth} AS INTEGER) - CAST(depth_val AS INTEGER)) * 4) OR is_capture = 1 OR is_check_val = 1 OR gives_check_val = 1)`;
}

export function getFFPFilterSQL(options, depth, maxDepth, alpha, beta) {
    if (options.useFFP === false || (maxDepth - depth > 2)) {
        return '';
    }
...
export function getRFPUpdateSQL(margin, alpha, beta) {
    const condition = getNMPConditionSQL(margin, alpha, beta);
    return `
        UPDATE search_tree 
        SET minimax_eval = static_eval 
        WHERE id IN (SELECT id FROM frontier_nodes WHERE ${condition})
    `;
}

export function getRFPDeleteSQL(margin, alpha, beta) {
    const condition = getNMPConditionSQL(margin, alpha, beta);
    return `DELETE FROM frontier_nodes WHERE ${condition} RETURNING 1`;
}

export function getLMRCheckSQL(isWhiteTurn, pAlpha, pBeta) {
    if (isWhiteTurn && pAlpha !== null) {
        return `SELECT 1 FROM search_tree WHERE depth=1 AND minimax_eval > ${pAlpha} AND id IN (SELECT parent_id FROM batch_d2_nodes) LIMIT 1`;
    } else if (!isWhiteTurn && pBeta !== null) {
        return `SELECT 1 FROM search_tree WHERE depth=1 AND minimax_eval < ${pBeta} AND id IN (SELECT parent_id FROM batch_d2_nodes) LIMIT 1`;
    }
    return '';
}

export function getLMRPruneSQL(isWhiteTurn, pAlpha, pBeta) {
    const pruneCondition = isWhiteTurn ? `minimax_eval > ${pAlpha}` : `minimax_eval < ${pBeta}`;
    return `
...
export function getMoveOrderingScoreSQL(options, depth) {
    let orderParts = [];

    // 1. Transposition Table best move
    if (options.useTT !== false) {
        orderParts.push(`(CASE WHEN tt.best_move_from = m.from_sq AND tt.best_move_to = m.to_sq THEN ${ORDER_TT_OFFSET} ELSE 0 END)`);
    }

    // 2. MVV-LVA Captures
    if (options.useMVVLVA !== false) {
        orderParts.push(`(CASE WHEN m.is_capture = 1 THEN ${ORDER_CAPTURE_OFFSET} + ${getMVVLVAScoreSQL('m.captured_piece', 'm.piece')} ELSE 0 END)`);
    } else {
        orderParts.push(`(CASE WHEN m.is_capture = 1 THEN ${ORDER_CAPTURE_OFFSET} ELSE 0 END)`);
    }
    ...
    // 5. Checks and Gives-Checks
    orderParts.push(`(CASE WHEN m.is_capture = 0 AND (c.is_check = 1 OR ${getGivesCheckSQL('m', 'c')} = 1) THEN ${ORDER_CHECK_OFFSET} ELSE 0 END)`);

    // 6. Killer Moves
    if (options.useKillers !== false) {
        orderParts.push(`(CASE WHEN EXISTS(SELECT 1 FROM killer_moves km WHERE km.depth = (CAST(${depth} AS INTEGER) - 1) AND km.from_sq = m.from_sq AND km.to_sq = m.to_sq) THEN ${ORDER_KILLER_OFFSET} ELSE 0 END)`);
    }

    // 7. History Moves
    if (options.useHistory !== false) {
        orderParts.push(`COALESCE(h.score, 0)`);
    }

    // 8. Piece-Square Tables (PST)
    if (options.usePST !== false) {
        orderParts.push(`COALESCE(pst.value, 0)`);
    }
...
export function getTTJoinSQL(options) {
    if (options.useTT === false) {
        return '';
    }
    return 'LEFT JOIN transposition_table tt ON (tt.board_hash = c.board_hash)';
}

export function getMoveOrderingHeuristicJoinsSQL(options) {
    let joins = [];

    // 1. History Moves Joins
    if (options.useHistory !== false) {
        joins.push(`LEFT JOIN history_moves h ON (h.piece = m.piece AND h.to_sq = m.to_sq)`);
    }

    // 2. PST Joins
    if (options.usePST !== false) {
        joins.push(`LEFT JOIN pst_values pst ON (pst.piece = m.piece AND pst.square = m.to_sq)`);
    }
...
export function getPersistentExpansionSQL(sourceTable, targetTable, depth, maxDepth, alpha, beta, options = {}) {
    ...
    const lmpFilter = getLMPFilterSQL(options, maxDepth);
    const ffpFilter = getFFPFilterSQL(options, depth, maxDepth, alpha, beta);
    const orderScore = getMoveOrderingScoreSQL(options, depth);
    const ttJoin = getTTJoinSQL(options);
    const heuristicJoins = getMoveOrderingHeuristicJoinsSQL(options);
    ...
    SELECT
        ...
        castling_rights, active_turn, ep_sq, is_ep, static_eval, board_hash, wK_sq, bK_sq, all_pieces,
    ...
    SELECT *, ${getOrSQL(['wK_bb', 'wQ_bb', 'wR_bb', 'wB_bb', 'wN_bb', 'wP_bb', 'bK_bb', 'bQ_bb', 'bR_bb', 'bB_bb', 'bN_bb', 'bP_bb'])} as all_pieces FROM (
        SELECT
            ...
            mr.from_sq, mr.to_sq, mr.piece, mr.captured_piece, mr.is_castle, mr.is_promo, mr.is_capture, mr.promo_piece, mr.is_ep,
            ...
            CAST(CASE WHEN ABS(mr.piece) = ${PIECES.P} AND ABS(mr.from_sq - mr.to_sq) = 16 THEN (mr.from_sq + mr.to_sq) / 2 ELSE -1 END AS TINYINT) as ep_sq
    ...
    WITH evaluations AS (
        ...
        LEFT JOIN pst_values pst_c ON pst_c.piece = scored.captured_piece AND pst_c.square = (CASE WHEN scored.is_ep = 1 THEN (CASE WHEN scored.active_turn_parent = ${TURNS.WHITE} THEN scored.to_sq - 8 ELSE scored.to_sq + 8 END) ELSE scored.to_sq END)
    )
    SELECT
        nextval('seq_search_tree_id') as id,
        parent_id, depth, active_turn, active_turn_parent,
        from_sq, to_sq, piece, captured_piece, is_castle, is_promo, is_capture, promo_piece, is_ep, ep_sq,
    ...
    SELECT
        ...
        wK_bb, wQ_bb, wR_bb, wB_bb, wN_bb, wP_bb,
        bK_bb, bQ_bb, bR_bb, bB_bb, bN_bb, bP_bb,
        castling_rights, active_turn, ep_sq, is_ep,
    ...
    SELECT
        id, parent_id, depth, from_sq, to_sq, piece, is_castle, is_promo, is_capture, captured_piece, promo_piece,
        wK_bb, wQ_bb, wR_bb, wB_bb, wN_bb, wP_bb, bK_bb, bQ_bb, bR_bb, bB_bb, bN_bb, bP_bb,
        castling_rights, active_turn, ep_sq, is_ep, static_eval, static_eval as minimax_eval, board_hash, wK_sq, bK_sq, all_pieces,
        my_pieces, opponent_pieces, active_king_sq, passive_king_sq, is_check
    FROM final_expanded
...
export function getQSInitSQL(qsTable, targetDepth) {
    ...
    SELECT
        ...
        s.castling_rights, s.active_turn, s.ep_sq, s.is_ep,
...
export function getQSExpansionSQL(qsFrontier, qsNextFrontier, qsTreeTable, qsDepth) {
    ...
    SELECT
        ...
        c.is_check as is_check_parent, c.ep_sq,
        m.from_sq, m.to_sq, m.piece, m.captured_piece, m.is_castle, m.is_promo, m.is_capture, m.promo_piece, m.is_ep
    ...
    FROM (
        SELECT
            ...
            mr.is_castle, mr.is_promo, mr.is_capture, mr.promo_piece, mr.is_ep,
    ...
    SELECT
        ...
        er.castling_rights, er.wK_sq, er.bK_sq, er.all_pieces, er.ep_sq, er.is_ep,
        ...
        (
            ...
            - CASE WHEN er.is_capture = 1 THEN COALESCE((SELECT value * SIGN(piece) FROM pst_values WHERE piece = er.captured_piece AND square = (CASE WHEN er.is_ep = 1 THEN (CASE WHEN er.active_turn_parent = ${TURNS.WHITE} THEN er.to_sq - 8 ELSE er.to_sq + 8 END) ELSE er.to_sq END)), 0) ELSE 0 END
...
// =============================================================================
// SEARCH LOOP HELPERS (moved from sessions.js)
// =============================================================================

/**
 * Generates the null-move pruning condition.
 *
 * @param {number} margin   – NMP margins
 * @param {number} loopAlpha
 * @param {number} loopBeta
...
/**
 * Swaps the search frontier: replaces frontier_nodes with next_frontier_nodes.
 * Used to advance the iterative, level-by-level expansion of the search tree.
 *
...
/**
 * Scores forced mates / draws for nodes that were expanded but produced no
 * legal moves (is_check = 1 → checkmate, else stalemate / no legal moves).
 *
 * @param {number} targetDepth – current search depth
 * @param {boolean} isBatching – when true, skips depth=1 nodes (handled separately per batch)
 * @returns {string} SQL statement
 */
...
/**
 * Initialises unresolved search-tree leaves with their static evaluation.
 *
 * @returns {string} SQL statement
 */
export function getInitializeLeavesSQL() {
    return `UPDATE search_tree SET minimax_eval = static_eval WHERE minimax_eval IS NULL`;
}

/**
 * Seeds the PV (principal variation) line into the frontier so the next
 * expansion iteration starts from the best line found so far.
 *
 * @param {number} pvId – id of the PV-related root/child node
 * @returns {string} SQL statement
 */
...
/**
 * Inserts all non-PV root children (depth=1) into parent_nodes so the
 * remaining root moves can be searched in batches (captures/quiet chunks).
 *
 * @param {number} pvId – id of the PV root child to exclude
 * @returns {string} SQL statement
 */
